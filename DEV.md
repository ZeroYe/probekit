...
采集器 (ICMP/DNS/SNMP/Port/HTTP)
    ...
    │  ICMP: 持久化 socket + 信号量(concurrency)并发控制, 支持 targets_file
    ▼
每个模块拥有独立的 Pipeline + VMPusher:
    │
    ├─► Registry.Store(key, metrics)     ← 共享 Registry, 供 MCP 查询
    │
    └─► input chan ─► RingBuffer         ← 解耦采集与推送
                    ─► Batcher           ← Prometheus 文本格式化
                    ─► HTTP POST         ← VictoriaMetrics /api/v1/import/prometheus
                        每个模块的 flush_interval / batch_size / buffer_size 独立可配
                        模块配置中对应字段为 0 时继承全局 victoria_metrics 默认值

MCP 服务 (:9801)
    ├─► /mcp     → StreamableHTTPServer (可选鉴权)
    ├─► /metrics → 自身指标处理器 (无鉴权)
    ├─► /upload  → 文件上传端点 (可选鉴权, 返回本地路径)
    └─► 6 个工具 (get_targets, get_metrics, add_target, batch_add_targets,
                   remove_target, reload_config)
```
...
ICMP 模块支持 `targets_file` 字段，将大量目标存放在独立 YAML 文件中：
...
MCP `batch_add_targets` 工具在模块配置了 `targets_file` 时，新目标会追加到外部文件；否则追加到内联 `targets`。

### 批量生成脚本

`scripts/gen-icmp-targets.sh` 用于大规模生成 ICMP 目标 YAML，支持三种输入模式：

```bash
# 模式 1: 标准输入（每行一个 IP）
cat ips.txt | ./scripts/gen-icmp-targets.sh

# 模式 2: CIDR 段
./scripts/gen-icmp-targets.sh --cidr 10.0.0.0/24 --label region=cn-beijing

# 模式 3: CSV 文件（每行可带独立标签）
./scripts/gen-icmp-targets.sh --csv targets.csv
```

CSV 格式：`ip,interval,count,timeout,labels`，其中 `labels` 列用 `|` 分隔键值对。
...
每个模块拥有自己的 `output.Pipeline` 和 `VMPusher`，配置中可独立指定：

```yaml
# icmp.yaml
flush_interval: 10s    # 覆盖全局, 0=继承全局
batch_size: 1000
buffer_size: 20000
```

实现方式：每个模块结构体实现 `EffectiveVM(global VMConfig) VMConfig` 方法，字段为 0 时返回全局默认值。
